...
from src.config import (
    ...
    ALL_FREE_PAIRS, ALL_PREMIUM_PAIRS,
...
def admin_dashboard():
    ...
    content = f"""
    <div class="grid">
      <div class="card"><h3>Signals Today</h3><div class="val">{stats.get('total',0)}</div></div>
      <div class="card"><h3>Win Rate</h3><div class="val" style="color:{'#22c55e' if stats.get('winrate',0)>=60 else '#ef4444'}">{stats.get('winrate',0)}%</div></div>
      <div class="card"><h3>TP Hits</h3><div class="val" style="color:#22c55e">{stats.get('tp_hits',0)}</div></div>
      <div class="card"><h3>SL Hits</h3><div class="val" style="color:#ef4444">{stats.get('sl_hits',0)}</div></div>
      <div class="card"><h3>Free Subs</h3><div class="val">{stats.get('free_subs',0)}</div></div>
      <div class="card"><h3>Premium Subs</h3><div class="val" style="color:#f59e0b">{stats.get('premium_subs',0)}</div></div>
    </div>
    ...
    <div class="section">
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap">
        ...
        <a href="/admin/resume" class="btn btn-primary">▶️ Resume Signals</a>
...
def admin_subscribers():
    ...
    content = f"""
    <div class="grid">
      <div class="card"><h3>Free Subscribers</h3><div class="val">{len(free)}</div></div>
      <div class="card"><h3>Premium Subscribers</h3><div class="val" style="color:#f59e0b">{len(premium)}</div></div>
      <div class="card"><h3>Total</h3><div class="val">{len(free)+len(premium)}</div></div>
    </div>
    <div class="section">
...
@admin_required
def admin_grant(tid):
    from src.database import upgrade_subscriber
...
def admin_revoke(tid):
    from src.database import downgrade_subscriber
...
def admin_grant_form():
    from src.database import upgrade_subscriber, downgrade_subscriber
    ...
    if request.method == "POST":
        ...
        if msg:
            if channel in ("free","both"):
                send_telegram(f"📢 <b>Veda Trader Update</b>\n\n{msg}", chat_id=FREE_CHANNEL_ID)
...
def admin_pause():
    ...
    send_telegram("⏸ <b>Signals paused by admin.</b> We'll be back shortly.", chat_id=FREE_CHANNEL_ID)
    return redirect(url_for("admin_dashboard"))
...
def admin_resume():
    ...
    flash("▶️ Signals resumed.")
    send_telegram("▶️ <b>Signals are back!</b> Watch for new entries.", chat_id=FREE_CHANNEL_ID)
    return redirect(url_for("admin_dashboard"))
...
def _handle_session_change(sess: str):
    ...
    # Broker reminder 5 mins before open
...
def scan_markets():
    global session_signals, last_session_alerted, last_summary_hour
    ...
    # Evaluate existing signals (TP/SL check)
    try:
    ...
    if weekend:
        # Only scan crypto for premium on weekends
        if not PREMIUM_CHANNEL_ID:
            return
        from src.config import PREMIUM_CRYPTO
    ...
    # AI hourly check → suggest pause if poor performance
    if now.hour != last_summary_hour and now.minute < 5:
        ...
        if pause:
            ...
            send_telegram("⏸ <b>Signals temporarily paused.</b> Our team is reviewing market conditions.", chat_id=FREE_CHANNEL_ID)
        else:
            # Send AI summary to admin every 4 hours
            if now.hour % 4 == 0 and now.minute < 5:
                summary = generate_admin_summary(stats)
                send_admin(f"📊 <b>Hourly Summary</b>\n\n{summary}")
    ...
    for pair in pairs_to_scan:
        ...
        # Don't repeat same direction twice in a row
        from src.database import get_last_signal_direction
    ...
    print(f"[SCAN] {now.strftime('%H:%M')} | {sess} | scanned={scanned} generated={generated} ✓{sent_ok} ✗{sent_fail}")
        ...
        if r.status_code == 200:
            ...
            if data.get("ok") and data.get("result"):
                for upd in data["result"]:
                    ...
                    try:
                        # Check if it's a subscriber question (non-command message)
                        msg  = upd.get("message", {})
                        ...
                        if text and not text.startswith("/"):
                            # AI auto-reply to subscriber questions
                            stats = get_daily_stats()
